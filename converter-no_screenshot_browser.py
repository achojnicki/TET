...
table {
	...
	padding: 0px;
	border-spacing: 0px;
	background: url("data:image/gif;base64,<_bg_>");
}
tr {
	margin: 0px;
	padding: 0px;
...
"""
	...
	def __init__(self):
		...
		with open(NOISE, 'rb') as file:
			self._noise=b64encode(file.read()).decode('utf-8')

		self._data=self._data.replace('<_bg_>', self._noise)
